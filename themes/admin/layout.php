...
<head>
    ...
    <style>
        :root {
            --primary-color:
                <?= \Core\Database::getSetting('primary_color', '#1d4ed8') ?>
            ;
        }
        ...
        h3,
        .heading-font {
            font-family: 'K2D', 'Sarabun', sans-serif !important;
        }
    ...
    <style>
        ...
        table.dataTable thead th {
            @apply px-6 py-5 text-[11px] font-bold text-slate-500 uppercase tracking-[0.2em] bg-slate-50/50 border-b border-slate-100 heading-font !important;
        }
...
<body class="bg-gray-50/50">
    ...
    <button data-drawer-target="separator-sidebar" data-drawer-toggle="separator-sidebar"
        ...
        class="inline-flex items-center p-2 mt-4 ms-4 text-sm text-gray-500 rounded-xl sm:hidden hover:bg-white focus:outline-none shadow-sm border border-gray-200">
    ...
    if (isset($_SESSION['user_id'])) {
        ...
        // Check for pending repairs (for Admin/Staff/Officer)
    ...
    <aside id="separator-sidebar"
        ...
        <div class="h-full px-4 py-8 overflow-y-auto bg-white border-r border-gray-100 shadow-xl shadow-gray-200/50">
            <div class="flex items-center px-4 mb-10 group">
                ...
                <div>
                    <h1 class="text-xl font-bold tracking-tight text-slate-800 heading-font truncate border-none">
                        <?= $siteName ?></h1>
                    <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">Admin Management
                    </p>
                </div>
            </div>
            ...
            <nav class="space-y-1.5">
                ...
                $menuItems = array_merge($menuItems, [
                    ...
                    [
                        ...
                        'badge' => $unreadSarabanCount > 0 ? $unreadSarabanCount : null,
                        'submenu' => [
                ...
                foreach ($menuItems as $item):
                    if (isset($item['submenu'])) {
                        ...
                        <div x-data="{ open: <?= $anySubActive ? 'true' : 'false' ?> }" class="space-y-1">
                            <button @click="open = !open"
                                class="w-full flex items-center justify-between px-4 py-3.5 text-sm font-bold transition-all duration-200 rounded-2xl group <?= $anySubActive ? 'bg-primary/5 text-primary' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-900' ?>">
                                <div class="flex items-center">
                                    <svg class="w-5 h-5 mr-3 <?= $anySubActive ? 'text-primary' : 'text-slate-400 group-hover:text-primary transition-colors' ?>"
                                ...
                                <div class="flex items-center">
                                    <?php if (isset($item['badge'])): ?>
                                        <span
                                            class="mr-2 px-2 py-0.5 bg-rose-500 text-white text-[10px] font-bold rounded-full"><?= $item['badge'] ?></span>
                                    <?php endif; ?>
                            ...
                            <div x-show="open" x-transition.opacity class="pl-12 space-y-1">
                                <?php foreach ($item['submenu'] as $sub):
                                    ...
                                    ?>
                                    <a href="<?= url($sub['url']) ?>"
                                        class="flex items-center py-2 text-xs font-medium transition-colors <?= $subActive ? 'text-primary font-bold' : 'text-slate-400 hover:text-slate-900' ?>">
                                        <?php if (isset($sub['icon'])): ?>
                                            <svg class="w-3.5 h-3.5 mr-2.5 <?= $subActive ? 'text-primary' : 'text-slate-300' ?>"
                                                fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                    d="<?= $sub['icon'] ?>"></path>
                                            </svg>
                                        ...
                                        <div class="flex-1 flex justify-between items-center min-w-0">
                                            ...
                                            <?php if (isset($sub['badge'])): ?>
                                                <span
                                                    class="px-1.5 py-0.5 bg-rose-500 text-white text-[9px] font-bold rounded-full"><?= $sub['badge'] ?></span>
                                            <?php endif; ?>
                                        </div>
                                    </a>
                                <?php endforeach; ?>
                    ...
                    ?>
                    <a href="<?= url($item['url']) ?>"
                        class="flex items-center px-4 py-3.5 text-sm font-bold transition-all duration-200 rounded-2xl group <?= $active ? 'nav-item-active' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-900' ?>">
                        <svg class="w-5 h-5 mr-3 <?= $active ? 'text-white' : 'text-slate-400 group-hover:text-primary transition-colors' ?>"
                            fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <?php if (\Core\Security::checkRole('admin')): ?>
                <a href="<?= url('/settings') ?>"
                    class="flex items-center px-4 py-3.5 text-sm font-bold text-slate-500 hover:bg-slate-50 hover:text-slate-900 rounded-2xl transition-all group <?= $currentPath === '/settings' ? 'bg-slate-50 text-slate-900' : '' ?>">
                    <svg class="w-5 h-5 mr-3 text-slate-400 group-hover:rotate-45 transition-transform" fill="none"
                ...
                <a href="<?= url('/settings/home-editor') ?>"
                    class="flex items-center px-4 py-3.5 text-sm font-bold text-slate-500 hover:bg-slate-50 hover:text-slate-900 rounded-2xl transition-all group <?= $currentPath === '/settings/home-editor' ? 'bg-slate-50 text-slate-900' : '' ?>">
                    <svg class="w-5 h-5 mr-3 text-slate-400 group-hover:scale-110 transition-transform" fill="none"
                        stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <a href="<?= url('/personnel/departments') ?>"
                    class="flex items-center px-4 py-3.5 text-sm font-bold text-slate-500 hover:bg-slate-50 hover:text-slate-900 rounded-2xl transition-all group <?= $currentPath === '/personnel/departments' ? 'bg-slate-50 text-slate-900' : '' ?>">
                    <svg class="w-5 h-5 mr-3 text-slate-400 group-hover:scale-110 transition-transform" fill="none"
                        stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <a href="<?= url('/settings/popups') ?>"
                    class="flex items-center px-4 py-3.5 text-sm font-bold text-slate-500 hover:bg-slate-50 hover:text-slate-900 rounded-2xl transition-all group <?= $currentPath === '/settings/popups' ? 'bg-slate-50 text-slate-900' : '' ?>">
                    <svg class="w-5 h-5 mr-3 text-slate-400 group-hover:scale-110 transition-transform" fill="none"
                        stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <a href="<?= url('/settings/menu') ?>"
                    class="flex items-center px-4 py-3.5 text-sm font-bold text-slate-500 hover:bg-slate-50 hover:text-slate-900 rounded-2xl transition-all group <?= $currentPath === '/settings/menu' ? 'bg-slate-50 text-slate-900' : '' ?>">
                    <svg class="w-5 h-5 mr-3 text-slate-400 group-hover:scale-110 transition-transform" fill="none"
                        stroke="currentColor" viewBox="0 0 24 24">
                ...
                <a href="<?= url('/settings/footer-editor') ?>"
                    class="flex items-center px-4 py-3.5 text-sm font-bold text-slate-500 hover:bg-slate-50 hover:text-slate-900 rounded-2xl transition-all group <?= $currentPath === '/settings/footer-editor' ? 'bg-slate-50 text-slate-900' : '' ?>">
                    <svg class="w-5 h-5 mr-3 text-slate-400 group-hover:scale-110 transition-transform" fill="none"
                        stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <a href="<?= url('/auth/logout') ?>"
                class="flex items-center px-4 py-3.5 text-sm font-bold text-red-500 hover:bg-red-50 rounded-2xl transition-all group">
                <svg class="w-5 h-5 mr-3 text-red-400 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
    ...
    <div class="p-4 sm:ml-72 min-h-screen">
        <div class="p-6 md:p-10">
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between mb-10 space-y-4 md:space-y-0">
                <div>
                    <h2 class="text-4xl font-extrabold text-slate-900 heading-font tracking-tight">
                        <?= $title ?? 'Dashboard' ?></h2>
                    <nav class="flex mt-2 text-xs font-bold text-slate-400 uppercase tracking-widest"
                        aria-label="Breadcrumb">
                        <ol class="inline-flex items-center space-x-1">
                            <li class="inline-flex items-center"><a href="<?= url('/dashboard') ?>"
                                    class="hover:text-primary">Admin</a></li>
                            <li>
                                <div class="flex items-center"><svg class="w-3 h-3 mx-1 text-slate-300"
                                        fill="currentColor" viewBox="0 0 20 20">
                                        <path
                                            d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z">
                                        </path>
                                    </svg> <span class="text-slate-300"><?= $title ?? 'Dashboard' ?></span></div>
                            </li>
                        </ol>
                    </nav>
                </div>
                <div class="flex items-center space-x-4">
                    <div class="flex flex-col items-end mr-3">
                        <span
                            class="text-sm font-bold text-slate-900 outfit leading-none"><?= $_SESSION['user_name'] ?? 'Admin' ?></span>
                    ...
                    <div
                        class="w-12 h-12 bg-white rounded-2xl shadow-sm border border-gray-100 flex items-center justify-center text-primary font-bold">
            ...
            <footer
                ...
                <div class="mt-6 md:mt-0 text-center md:text-right">
                    ...
                    <div class="text-[11px] font-bold text-slate-400 mt-0.5 uppercase tracking-widest">โรงเรียนลำปลายมาศ
                    </div>
                </div>
            </footer>
        </div>
