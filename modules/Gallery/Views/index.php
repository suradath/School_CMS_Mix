<div class="mb-6 flex justify-between items-center text-right">
    <a href="/gallery/create" class="inline-flex items-center px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-lg transition shadow-md">
...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
    ...
    <div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-lg transition group">
        <div class="relative h-48 overflow-hidden">
            <img src="<?= $album['cover_image'] ?: 'https://via.placeholder.com/400x300?text=No+Cover' ?>" 
            ...
            <div class="absolute inset-0 bg-black bg-opacity-20 opacity-0 group-hover:opacity-100 transition flex items-center justify-center">
                <a href="/gallery/view/<?= $album['id'] ?>" class="bg-white text-blue-600 px-4 py-2 rounded-lg font-bold text-sm shadow-xl">ดูอัลบั้ม</a>
            </div>
        </div>
        <div class="p-5">
            ...
            <div class="mt-4 pt-4 border-t border-gray-50 flex justify-between items-center text-[10px] text-gray-400 uppercase tracking-widest font-bold">
                ...
                <div class="space-x-2">
                    <a href="/gallery/edit/<?= $album['id'] ?>" class="hover:text-blue-600 transition">แก้ไข</a>
                    <a href="/gallery/delete/<?= $album['id'] ?>" class="hover:text-red-600 transition" onclick="return confirm('ลบอัลบั้มนี้?')">ลบ</a>
                </div>
            </div>
        </div>
    </div>
